...
function createFlameBarrier(monster) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    // Randomize particle position around the barrier
...
function createHazardSpikes(monsterPosition) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    // Randomize spike position within maze
    let validPosition = false;
    let attempts = 0;
    while (!validPosition && attempts < 100) {
      ...
      // Ensure spike is not too close to walls or monster
    ...
    // Add pulsing animation to spike
    ...
    spike.userData.checkPlayerProximity = () => {
      const distance = spike.position.distanceTo(camera.position);
      if (distance < 1) {
        gameEnd(false);
      }
    };
